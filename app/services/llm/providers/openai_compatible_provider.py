...
使用OpenAI兼容接口调用各种支持OpenAI API格式的服务，支持视觉分析和文本生成
"""
...
class OpenAICompatibleVisionProvider(VisionModelProvider):
    """通用OpenAI兼容视觉模型提供商"""
    ...
    async def analyze_images(self,
        ...
        Returns:
            分析结果列表
        """
        logger.info(f"开始分析 {len(images)} 张图片，使用OpenAI兼容API")
    ...
    async def _analyze_batch(self, batch: List[PIL.Image.Image], prompt: str) -> str:
        ...
        if response.choices and len(response.choices) > 0:
            ...
            raise APICallError("OpenAI兼容API返回空响应")
    ...
    async def generate_text(self,
        ...
        try:
            ...
            if response.choices and len(response.choices) > 0:
                ...
                logger.debug(f"OpenAI兼容API调用成功，消耗tokens: {response.usage.total_tokens if response.usage else 'N/A'}")
                return content
            else:
                raise APICallError("OpenAI兼容API返回空响应")
            ...
            if "response_format" in str(e) and response_format == "json":
                ...
                raise APICallError(f"OpenAI兼容API请求失败: {str(e)}")
                
        except Exception as e:
            logger.error(f"OpenAI兼容API调用失败: {str(e)}")
            raise APICallError(f"OpenAI兼容API调用失败: {str(e)}")
