...
def combine_clip_videos(
    ...
    # 创建视频处理配置字典列表 - 修复重复片段问题
    # 确保每个视频片段只处理一次，避免重复添加
    processed_paths = set()  # 跟踪已处理的视频路径，防止重复处理相同文件
    for i, (video_path, video_ost) in enumerate(zip(video_paths, video_ost_list)):
        # 检查视频路径是否已处理过，避免重复
        if video_path in processed_paths:
            logger.warning(f"跳过重复的视频片段路径: {video_path}")
            continue
        ...
        processed_paths.add(video_path)  # 标记为已处理
    ...
    try:
        ...
        try:
            ...
            # 创建音频中间文件
            audio_files = []
            processed_audio_paths = set()  # 跟踪已处理的音频路径，防止重复处理
            for i, segment in enumerate(audio_segments):
                # 检查音频路径是否已处理过，避免重复
                if segment["path"] in processed_audio_paths:
                    logger.warning(f"跳过重复的音频片段路径: {segment['path']}")
                    continue
                ...
                processed_audio_paths.add(segment["path"])  # 标记为已处理
            ...
            # 获取每个视频片段的时长 - 修复重复计算问题
            # 使用processed_videos而不是原始video_paths来计算时长
            processed_video_dict = {video["index"]: video for video in processed_videos}
            for i in range(len(processed_videos)):  # 确保按顺序处理
                if i not in processed_video_dict:
                    continue
                    
                video = processed_video_dict[i]
                duration_cmd = [
                    'ffprobe', '-v', 'error',
            ...
            processed_audio_files = set()  # 跟踪已处理的音频文件，防止重复添加
            for timing in audio_timings:
                if timing["file"] in processed_audio_files:
                    logger.warning(f"跳过重复的音频文件: {timing['file']}")
                    continue
                audio_inputs.extend(['-i', timing["file"]])
                processed_audio_files.add(timing["file"])  # 标记为已处理
