...
def merge_materials(
    ...
    # 智能音量调整（可选功能）- 修改为使用前端用户输入的值
    # 移除智能音量分析逻辑，直接使用用户设置的音量值
    # 原来的智能音量分析逻辑已被注释掉
    ...
    # 直接使用前端传递的音量值，确保原片播放音量和解说音量均使用前端传递的数值
    logger.info(f"使用前端传递的音量值 - TTS: {voice_volume}, 原声: {original_audio_volume}")
