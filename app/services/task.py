...
def start_subclip(task_id: str, params: VideoClipParams, subclip_path_videos: dict = None):
    ...
    # 使用统一裁剪后的视频片段
    video_clips = []
    video_ost = []  # 同时构建OST列表，确保与video_clips一一对应
    
    for new_script in new_script_list:
        video_path = new_script.get('video')
        ost_value = new_script.get('OST', 0)  # 获取OST值，默认为0
        
        if video_path and os.path.exists(video_path):
            video_clips.append(video_path)
            video_ost.append(ost_value)  # 同步添加OST值
        else:
    ...
    # 应用用户设置和优化建议的组合
    # 修改为优先使用用户设置的音量值，而不是默认值
    # 如果用户设置了非默认值，优先使用用户设置
    if hasattr(params, 'tts_volume') and params.tts_volume is not None:
        final_tts_volume = params.tts_volume
    else:
        final_tts_volume = optimized_volumes['tts_volume']

    # 关键修复：如果有原声片段，保持原声音量为1.0，确保与原视频音量一致
    # 但也要尊重用户设置的值
    if has_original_audio_segments:
        # 如果有原声片段，但用户设置了特定的原声音量，则使用用户设置的值
        if hasattr(params, 'original_volume') and params.original_volume is not None and params.original_volume != 1.2:  # 1.2是默认值
            final_original_volume = params.original_volume
            logger.info(f"检测到原声片段，但使用用户设置的原声音量: {final_original_volume}")
        else:
            final_original_volume = 1.0  # 保持原声音量不变
            logger.info("检测到原声片段，原声音量设置为1.0以保持与原视频一致")
    else:
        if hasattr(params, 'original_volume') and params.original_volume is not None:
            final_original_volume = params.original_volume
        else:
            final_original_volume = optimized_volumes['original_volume']

    if hasattr(params, 'bgm_volume') and params.bgm_volume is not None:
        final_bgm_volume = params.bgm_volume
    else:
        final_bgm_volume = optimized_volumes['bgm_volume']
...
def start_subclip_unified(task_id: str, params: VideoClipParams):
    ...
    if path.exists(video_script_path):
        try:
            with open(video_script_path, "r", encoding="utf-8") as f:
                ...
                # 验证脚本中每个片段都有唯一ID
                used_ids = set()
                for i, script_item in enumerate(list_script):
                    item_id = script_item.get('_id')
                    if item_id is None:
                        # 如果没有ID，分配一个唯一的ID
                        script_item['_id'] = i + 1
                        ...
                        while new_id in used_ids:
                            new_id += len(list_script)
                        script_item['_id'] = new_id
                    used_ids.add(script_item['_id'])
        except Exception as e:
            logger.error(f"无法读取视频json脚本，请检查脚本格式是否正确")
            raise ValueError("无法读取视频json脚本，请检查脚本格式是否正确")
    else:
    ...
    # 确保video_ost列表与video_clips长度一致，避免重复片段问题
    if len(video_clips) != len(video_ost):
        logger.warning(f"视频片段数量({len(video_clips)})与OST设置数量({len(video_ost)})不匹配")
        # 调整OST列表长度与视频片段数量一致
        if len(video_ost) < len(video_clips):
            # 如果OST列表较短，用默认值0填充
            video_ost.extend([0] * (len(video_clips) - len(video_ost)))
        else:
            # 如果OST列表较长，截断到与视频片段数量一致
            video_ost = video_ost[:len(video_clips)]
        logger.info(f"调整后OST列表长度: {len(video_ost)}")
