...
def test_vision_model_connection(api_key, base_url, model_name, provider, tr):
    ...
    elif provider.lower() == 'openai compatible':
        # 通用OpenAI兼容API测试
        try:
            headers = {
                "Authorization": f"Bearer {api_key}",
            ...
            response = requests.post(test_url, headers=headers, json=test_data, timeout=10)
            if response.status_code == 200:
                return True, tr("OpenAI兼容模型连接成功")
            else:
                return False, f"{tr('OpenAI兼容模型连接失败')}: HTTP {response.status_code}"
        except Exception as e:
            return False, f"{tr('OpenAI兼容模型连接失败')}: {str(e)}"
    else:
...
def render_vision_llm_settings(tr):
    ...
    # 视频分析模型提供商选择
    vision_providers = ['Siliconflow', 'Gemini', 'Gemini(OpenAI)', 'QwenVL', 'OpenAI', 'OpenAI Compatible']
    ...
    for i, provider in enumerate(vision_providers):
        ...
        if provider == 'Gemini(OpenAI)':
            provider_key = 'gemini(openai)'
        elif provider == 'OpenAI Compatible':
            provider_key = 'openai_compatible'
            
        if provider_key == saved_vision_provider:
    ...
    if vision_provider == 'Gemini(OpenAI)':
        vision_provider_key = 'gemini(openai)'
    elif vision_provider == 'OpenAI Compatible':
        vision_provider_key = 'openai_compatible'
    else:
        vision_provider_key = vision_provider.lower()
        
    config.app["vision_llm_provider"] = vision_provider_key
    st.session_state['vision_llm_providers'] = vision_provider_key

    # 获取已保存的视觉模型配置
    # 处理特殊的提供商名称映射
    if vision_provider_key == 'gemini(openai)':
        vision_config_key = 'vision_gemini_openai'
    elif vision_provider_key == 'openai_compatible':
        vision_config_key = 'vision_openai_compatible'
    else:
        vision_config_key = f'vision_{vision_provider_key}'
    ...
    if vision_provider_key == 'gemini':
    ...
    elif vision_provider_key == 'gemini(openai)':
        st_vision_base_url = st.text_input(
            tr("Vision Base URL"),
    ...
    elif vision_provider_key == 'qwenvl':
        st_vision_base_url = st.text_input(
            tr("Vision Base URL"), 
        ...
        st_vision_base_url = st.text_input(
            tr("Vision Base URL"),
            value=vision_base_url,
            help=tr("OpenAI兼容API端点，如: https://api.openai.com/v1")
        )
        st_vision_model_name = st.text_input(
            tr("Vision Model Name"),
            value=vision_model_name or "gpt-4o",
            help=tr("OpenAI兼容模型名称，默认: gpt-4o")
        )
    else:
            ...
            with st.spinner(tr("Testing connection...")):
                try:
                    success, message = test_vision_model_connection(
                        ...
                        provider=vision_provider_key,  # 使用正确的提供商键
                        tr=tr
                    )
    ...
    if st_vision_api_key:
        is_valid, error_msg = validate_api_key(st_vision_api_key, f"视频分析({vision_provider_key})")
        if is_valid:
    ...
    if st_vision_base_url:
        is_valid, error_msg = validate_base_url(st_vision_base_url, f"视频分析({vision_provider_key})")
        if is_valid:
    ...
    if st_vision_model_name:
        is_valid, error_msg = validate_model_name(st_vision_model_name, f"视频分析({vision_provider_key})")
        if is_valid:
    ...
    if config_changed and not validation_errors:
        try:
            ...
            if st_vision_api_key or st_vision_base_url or st_vision_model_name:
                st.success(f"视频分析模型({vision_provider_key})配置已保存")
        except Exception as e:
...
def render_text_llm_settings(tr):
    ...
    # 文案生成模型提供商选择
    text_providers = ['OpenAI', 'Siliconflow', 'DeepSeek', 'Gemini', 'Gemini(OpenAI)', 'Qwen', 'Moonshot', 'OpenAI Compatible']
    ...
    for i, provider in enumerate(text_providers):
        ...
        if provider_key == saved_text_provider:
            saved_provider_index = i
            break
    ...
    if text_provider == 'Gemini(OpenAI)':
        text_provider_key = 'gemini(openai)'
    elif text_provider == 'OpenAI Compatible':
        text_provider_key = 'openai_compatible'
    else:
        text_provider_key = text_provider.lower()
        
    config.app["text_llm_provider"] = text_provider_key

    # 获取已保存的文本模型配置
    if text_provider_key == 'gemini(openai)':
        text_config_key = 'text_gemini_openai'
    elif text_provider_key == 'openai_compatible':
        text_config_key = 'text_openai_compatible'
    else:
        text_config_key = f'text_{text_provider_key}'

    text_api_key = config.app.get(f"{text_config_key}_api_key")
    text_base_url = config.app.get(f"{text_config_key}_base_url")
    text_model_name = config.app.get(f"{text_config_key}_model_name")

    # 渲染文本模型配置输入框
    st_text_api_key = st.text_input(tr("Text API Key"), value=text_api_key, type="password")

    # 根据不同提供商设置默认值和帮助信息
    if text_provider_key == 'gemini':
    ...
    elif text_provider_key == 'gemini(openai)':
        st_text_base_url = st.text_input(
            tr("Text Base URL"),
        ...
        st_text_model_name = st.text_input(
            tr("Text Model Name"),
            value=text_model_name or "gpt-4o",
            help=tr("OpenAI兼容模型名称，默认: gpt-4o")
        )
    else:
            ...
            with st.spinner(tr("Testing connection...")):
                try:
                    success, message = test_text_model_connection(
                        ...
                        provider=text_provider_key,  # 使用正确的提供商键
                        tr=tr
                    )
    ...
    # 验证API密钥
    if st_text_api_key:
        is_valid, error_msg = validate_api_key(st_text_api_key, f"文案生成({text_provider_key})")
        if is_valid:
            config.app[f"{text_config_key}_api_key"] = st_text_api_key
    ...
    # 验证Base URL
    if st_text_base_url:
        is_valid, error_msg = validate_base_url(st_text_base_url, f"文案生成({text_provider_key})")
        if is_valid:
            config.app[f"{text_config_key}_base_url"] = st_text_base_url
            text_config_changed = True
        else:
            text_validation_errors.append(error_msg)
    ...
    if st_text_model_name:
        is_valid, error_msg = validate_model_name(st_text_model_name, f"文案生成({text_provider_key})")
        if is_valid:
            config.app[f"{text_config_key}_model_name"] = st_text_model_name
            text_config_changed = True
        else:
            text_validation_errors.append(error_msg)
    ...
    if text_config_changed and not text_validation_errors:
        try:
            ...
            if st_text_api_key or st_text_base_url or st_text_model_name:
                st.success(f"文案生成模型({text_provider_key})配置已保存")
        except Exception as e:
            st.error(f"保存配置失败: {str(e)}")
